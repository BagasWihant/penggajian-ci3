...
class SlipGaji extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/VslipGaji', $data);
    ...
    public function printGaji()
    {
        ...
        $res = '';
        foreach ($data as $key) {
            $dtjabatan = $this->GajiModel->cariData('id',$key->jabatan,'data_jabatan')->row();
            $gaji = $dtjabatan->gaji + $dtjabatan->tunj_transport + $dtjabatan->tunj_makan;
            $res .= "<h1>gaji</h1>nama : $key->nama_pegawai<br>
            nik :$key->nik<br>
            posisi  :$key->jabatan<br>
            Gaji    : $gaji<pagebreak>";
        }
        $data['isi'] = $res;
